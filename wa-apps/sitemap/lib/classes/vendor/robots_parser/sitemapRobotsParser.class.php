...
class sitemapRobotsParser
{
	...
	private $service_port = array();
	private $is_inline_derective = array();
	...
	protected function prepareRules()
	{
		...
		foreach ($this->rules as $userAgent => $directive)
		{
			foreach ($directive as $directiveName => $directiveValue)
			{
				...
				{
					$this->rules[$userAgent][$directiveName] = $directiveName == self::DIRECTIVE_DISALLOW
						? array_values(array_unique(array_filter($directiveValue, array($this, 'filterRootDisallowValue'))))
						: array_values(array_unique($directiveValue));
				}
			}
		}
	}
	...
	protected function parseURL($url)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					$parsed['port'] = $this->getPortByService($parsed['scheme'], 'tcp');
		...
		$parsed['custom'] = (isset($parsed['path']) ? $parsed['path'] : '/') . (isset($parsed['query']) ? '?'
				. $parsed['query'] : '');
	...
	private function explodeCleanParamRule($rule)
	{
		...
		$cleanParam['path'] = isset($array[1]) ? $this->encode_url(
			preg_replace('/[^A-Za-z0-9\.-\/\*\_]/', '', $array[1])
		) : '/*';
	...
	{
		$uaParser = new sitemapRobotsUserAgentParser($this->userAgent);
	...
	{
		if (!array_key_exists($rule, $this->is_inline_derective))
		{
			$is_inline = false;
			...
			$this->is_inline_derective[$rule] = $is_inline;
		}

		return $this->is_inline_derective[$rule];
	}
	...
	{
		$escaped = $this->prepareRegexRule($this->encode_url($rule));
	...
	public function getDelay($userAgent = null, $type = 'crawl-delay')
	{
		...
		{
			$this->log[] = "$directive directive (unofficial): Not found, fallback to " . self::DIRECTIVE_CRAWL_DELAY
				. ' directive';
	...
	public function render($eol = "\r\n")
	{
		...
		foreach ($input as $userAgent => $rules)
		{
			...
			foreach ($rules as $directive => $value)
			{
				...
				{
					// Shorter paths later
					usort($value, array($this, '_compareStringLength'));
	...
	public function _compareStringLength($a, $b)
	{
	...
	private function getPortByService($service, $proto)
	{
		$key = $service . '_' . $proto;
		if (!array_key_exists($key, $this->service_port))
		{
			$this->service_port[$key] = getservbyname($service, $proto);
		}

		return $this->service_port[$key];
	}

	private function filterRootDisallowValue($value)
	{
