 ...
 * @package vipnytt
 */
class sitemapRobotsUserAgentParser
{
	/**
	...
	private function validateProduct()
	{
		...
		{
			throw new sitemapException('Product string cannot be empty.');
		}
	...
	{
		foreach (array('mozilla', 'compatible', '(', ')',) as $string)
		{
			if (stripos($input, $string) !== false)
			{
				throw new sitemapException(
					'Invalid User-agent format (`' . trim($this->product . '/' . $this->version, '/')
					. '`). Examples of valid User-agents: `MyCustomBot`, `MyFetcher-news`, `MyCrawler/2.1` and `MyBot-images/1.2`. See also '
					. self::RFC_README
				);
			}
		}
	...
	private function validateVersion()
	{
		...
		if (!empty($this->version)
			&& (
				str_replace(
					'+',
					'',
					filter_var($this->version, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION)
				) !== $this->version
		...
		{
			throw new sitemapException(
				"Invalid version format (`$this->version`). See http://semver.org/ for guidelines. In addition, dev/alpha/beta/rc tags is disallowed. See also "
				. self::RFC_README
			);
		}
	...
	{
		while (
			!empty($this->version) && substr_count($this->version, '.') < 2
		)
		{
			$this->version .= '.0';
		}
		...
		asort($result);
		usort($result, array($this, '_compareStringLength'));
