...
class contactsSendpulseApi implements contactsSendpulseApiInterface {
    ...
    public function __construct( $userId, $secret, $storageType = 'file' ) {
        if( empty( $userId ) || empty( $secret ) ) {
        	...
        	exit(
        		json_encode(array(
        		"status"=>false, 
        		"message"=>_wp("Empty ID or SECRET")))
        	);
        ...
        if( empty( $this->token ) ) {
            if( !$this->getToken() ) {
            	exit(
            		json_encode(array(
            		"status"=>false, 
            		"message"=>_wp("Could not connect to api, check your ID and SECRET")))
            	);
