 ...
 */
class shopProductmotosPluginCollection extends shopProductsCollection
{
    public function getMotos()
    {
        $feature_id = (int)wa()->getSetting('feature_id', '', array('shop', 'productmotos'));
        ...
        foreach ($motos as $id => $b) {
            if (isset($rows[$id])) {
                $motos[$id]['count'] = $rows[$id];
            } else {
                unset($motos[$id]);
            }
        }
    ...
    public function setMotoSortProducts($sort)
    {
        $tmp = explode(' ', $sort);
        if (!isset($tmp[1])) {
            $tmp[1] = 'DESC';
