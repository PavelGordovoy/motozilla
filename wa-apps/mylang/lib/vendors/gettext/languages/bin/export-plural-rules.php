...
try {
    ...
    if (isset(Enviro::$outputFilename)) {
        echo call_user_func([Exporter::getExporterClassName(Enviro::$outputFormat), 'toFile'], $languages, Enviro::$outputFilename, ['us-ascii' => Enviro::$outputUSAscii]);
    } else {
        echo call_user_func([Exporter::getExporterClassName(Enviro::$outputFormat), 'toString'], $languages, ['us-ascii' => Enviro::$outputUSAscii]);
    }
} catch (Exception $x) {
...
class Enviro
{
    ...
    public static function showSyntax()
    {
        ...
        $array_map = [];
        foreach ($exporters as $key => $string) {
            $array_map[$key] = strlen($exporters[$key]);
        }
        $len = max($array_map);
