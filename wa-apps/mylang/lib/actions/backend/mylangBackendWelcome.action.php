...
class mylangBackendWelcomeAction extends waViewAction
{
    private $config;
    ...
    public function execute()
    {
        ...
        $app_settings_model->set('mylang', 'welcome', 1);
        if (waRequest::post()) {
            $app_settings_model = new waAppSettingsModel();

            $passed_steps = json_decode($app_settings_model->get('mylang', 'tutorial_passed_steps', '[]'), true);
            $passed_action = waRequest::post('passed_action', null, waRequest::TYPE_STRING);
            if ($passed_action && !in_array($passed_action, $passed_steps, true)) {
                $passed_steps[] = $passed_action;
                $app_settings_model->set('mylang', 'tutorial_passed_steps', json_encode($passed_steps));
            }

            $welcome = $app_settings_model->get('mylang', 'welcome', false);
            $app_settings_model->set('mylang', 'show_tutorial', 1);
            if ($welcome) {
    ...
    private function overview()
    {
        ...
        if (!isset($locale['iso3'], $countries[$locale['iso3']])) {
        ...
        $this->view->assign(array(
            ...
            'actions'           => mylangTutorialActions::getActions(true),
