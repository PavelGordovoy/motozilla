 ...
 */
class mylangTheme implements ArrayAccess
{
    /**
        ...
        if (empty($this->info)) {
            ...
            switch ($extension) {
                case 'xml':
                     ...
                     */
                    $ml_fields = ['name', 'description', 'about'];
                    ...
                    if ($files = $xml->{'files'}) {
                        ...
                        foreach ($files->children() as $file) {
                            ...
                            if (in_array(pathinfo($path, PATHINFO_EXTENSION), ['js', 'html', 'css'])) {
                    ...
                    if ($settings = $xml->{'settings'}) {
                        ...
                        foreach ($settings->children() as $setting) {
                            ...
                            if ($setting->{'options'}) {
                                $this->info['settings'][$var]['options'] = [];
                                foreach ($setting->{'options'}->children() as $option) {
                                    $s['options'][(string)$option['value']] = [];
     ...
     * @throws waException
     */
    public function addFile($path, $description = null, $options = [])
    {
        if ($description) {
        ...
        if (!in_array(pathinfo($path, PATHINFO_EXTENSION), ['css', 'js', 'html'])) {
            throw new waException("Unexpected file extension");
        }
        $options['custom'] = 1;
        $options['modified'] = 1;
        $this->setFiles([$path => $options]);
        return $this;
    }
    ...
    {
        $this->init();
        $this->setFiles([$file => ['modified' => true]]);
        ...
        $this->setFiles([$file => ['description' => $description]]);
        return $this->save();
    }
     ...
     */
    public function getXML($as_dom = false)
    {
...
XML;
        ...
        if (!$this->readonly && $this->changed && $this->path) {
            ...
            switch ($extension) {
                case 'xml':
                        ...
                        foreach ($this->changed['settings'] as $var => $changed) {
                            ...
                            if ($length && ($value = $value_items->item(0))) {
                                ...
                                if (ifset($this->settings[$var]['control_type']) == 'text') {
                                    $value->nodeValue = '';
                                    if (is_array($this->settings[$var]['value'])) {
                                        foreach ($this->settings[$var]['value'] as $key => $val) {
                                            $element = $dom->createElement('value', '');
                                            $element->setAttribute('locale', $key);
                                            $element->appendChild(new DOMCdataSection(ifempty($val, '')));
                    ...
                    if (!empty($this->changed['locales'])) {
                        //locales workaround
                        $query = "/theme/locales";
                        $localesOld = $xpath->query($query)->item(0);
                        $locales = $this->addNode($dom, $xpath, $theme, 'locales');
                        foreach ($this->changed['locales'] as $var => $changed) {
                            $locale = $dom->createElement('locale');
                            $locale->appendChild($dom->createElement('msgid', $var));
                            foreach ($changed as $lang=>$localizedString) {
                                $msgstr = $dom->createElement('msgstr', self::prepareField($localizedString));
                                $msgstr->setAttribute('locale', $lang);
                                $locale->appendChild($msgstr);
                            }
                            $locales->appendChild($locale);
                        }
                        $localesOld->parentNode->replaceChild($locales, $localesOld);
                        unset($this->changed['locales']);
                    }
     ...
     * @return waTheme
     */
    public function copy($id = null, $params = [])
    {
        if ($id) {
            self::verify($id);
        } else {
     ...
     * @return waTheme
     */
    public function fork($id, $params = [])
    {
    ...
    {
        if ($this->path_custom && in_array($this->type, [self::OVERRIDDEN, self::CUSTOM])) {
            $res = waFiles::delete($this->path_custom);
            $this->flush();
            return $res;
        } else {
...
HTACCESS;
    ...
    private function setFiles($file)
    {
        ...
        foreach ($file as $path => $properties) {
            ...
            if (!$properties) {
                ...
                if (!isset($this->info['files'][$path])) {
                    $this->info['files'][$path] = ['description' => [], 'custom' => true];
                }
    ...
    private function getMethod($offset, $type = 'get')
    {
        ...
        if (!isset($methods[$type][$offset])) {
            $methods[$type][$offset] = $type.preg_replace_callback('/(^|_)([a-z])/', [$this, 'getMethodCallback'], $offset);
    ...
    {
        $this->info['locales'] = $value;
        $this->changed['locales'] = $value;
        return $this->info['locales'];
    }

    public function getLocales($all = false)
    {
        $this->init();
        $locale = wa()->getLocale();
        $result = [];
        if ($all) {
            $result = $this->info['locales'];
            return $result;
        }
     ...
     * @return string
     */
    private static function getLocale($data = [], $locale = null)
    {
        if (!$locale) {
            $locale = wa()->getLocale();
        }
    ...
    {
        uasort($themes, [__CLASS__, 'sortThemesHandler']);
        return $themes;
    }
    ...
    private static function getRoutingRules($domains, $app_id, $theme_id)
    {
        ...
        if (!is_array($themes)) {
            $themes = [];
            $theme_types = ['desktop' => 'theme', 'mobile' => 'theme_mobile'];
    ...
    private function getAvailableId($apps = [])
    {
        $numerator = 0;
        $exists = null;
        if (empty($apps)) {
     ...
     * @return waTheme
     */
    public function duplicate($related = false, $options = [])
    {
        if (!empty($options['id'])) {
        ...
        if (class_exists('Archive_Tar')) {
            try {
                ...
                foreach ($xml->xpath('/theme/files/file') as $theme_file) {
                    ...
                    if (!in_array(pathinfo($theme_file['path'], PATHINFO_EXTENSION), ['html', 'js', 'css'])) {
                ...
                foreach ($files as $file) {
                    if ($extract_pattern && !preg_match("@^{$extract_pattern}(/|$)@", $file['filename'])) {
                        ...
                        if (preg_match('@(^|/)\\.htaccess$@', $file['filename'])) {
                            ...
                            if (!in_array(strtolower(basename($file['filename'])), ['theme.xml', 'build.php', '.htaccess', 'readme',])) {
                                self::throwThemeException('UNEXPECTED_FILE_TYPE', $file['filename']);
                            }
                        }
                        if ($extract_pattern) {
