 ...
 */
class multiformInputControl extends multiformFormBuilder
{
    ...
    public static function execute($name, $params)
    {
        ...
        $control .= self::addCustomParams(array('class', 'maxlength', 'value', 'placeholder', 'readonly', 'disabled'), $params);
        $control .= ">";
        return $control;
    }
    ...
    public function validate_field($field, $value, $form_fields, $post_fields)
    {
        ...
        if (!empty($field['validation']['unique'])) {
            ...
            if ($mrv->countByField(array("field_id" => $field['id'], "value" => $value))) {
                $result['errors']['messages']['input_unique_' . $field['id']] = sprintf(_wd(self::$app, "Value of the field \"%s\" is already exist. Please, specify unique value"), multiformHelper::secureString($field['title']));
            }
        }
    ...
    {
        return array(
            'control_type' => 'getInputFieldControl',
            'title' => _w('New textfield'),
            'properties' => array(
                'title' => array(
                    'title' => _w('Title'),
                    'value' => '',
