 ...
 */
class multiformRadioControl extends multiformFormBuilder
{
        ...
        if ($params['option_fields']) {
            ...
            foreach ($params['option_fields'] as $option_id => $option) {
                ...
                if (isset($params['record_value'])) {
                    if (isset($params['record_value'][$option_id])) {
                        $option['checked'] = 1;
                        if ($option['sort'] == 999999) {
                            $option['other_value'] = $params['record_value'][$option_id];
                        }
                    } else {
                        $option['checked'] = 0;
                    }
                ...
                $control .= "<input type=\"" . $params['type'] . "\" data-formula=\"" . multiformHelper::secureString($option['formula']) . "\" data-id=\"{$option_id}\" id=\"{$control_name}-{$option_id}\" name=\"{$control_name}[value]\" value=\"" . $option_id . "\"" . ($option['checked'] ? " checked" : "");
                if (empty($params['readonly']) && !empty($params['disabled'])) {
                    $control .= " disabled>";
                } else if (!empty($params['readonly'])) {
                    $control .= " disabled>";
                    if ($option['checked']) {
                        $control .= "<input type=\"hidden\"" . (!empty($params['disabled']) ? " disabled" : "") . " data-formula=\"" . multiformHelper::secureString($option['formula']) . "\" name=\"{$control_name}[value]\" value=\"" . $option_id . "\">";
                    }
                } else {
                    $control .= ">";
                }
                ...
                $control .= "<span>" . multiformHelper::secureString($option['value']) . "</span>";
                $control .= "</label>";
                // Вывод дополнительного текстового поля
                if ($option['sort'] == 999999) {
                    $control .= "<br><input type=\"text\"" . (!empty($params['disabled']) ? " disabled" : "") . " name=\"{$control_name}[other]\" class=\"multiform-other-field\" onfocus=\"$(this).closest('.multiform-gap-option').find(':radio').prop('checked', true)\"" . (isset($option['other_value']) ? " value = '" . multiformHelper::secureString($option['other_value']) . "'" : "") . ">";
                }
                $control .= "</div>";
            }
            $control .= "</div>";
    ...
    public function prepareOption_image_sizeSettingsParams(&$params, $field)
    {
    ...
    public function prepareOption_valuesSettingsParams(&$params, $field)
    {
        $params['field'] = $field;
    }

    public function GetOptionRadioSettingsControl($name, $params)
    {
        $control_name = multiformHelper::secureString($name);
        $control = "";
        $field = $params['field'];
        ...
        foreach ($options as $o) {
            ...
            $control .= "<tr class=\"" . ((!empty($field['options']['allow_other']) && $o['sort'] == '999999') ? "other-field" : "sortable") . "\" data-id=\"" . $o['id'] . "\">
                            ...
                            <td class=\"customize\">" . ((!empty($field['options']['allow_other']) && $o['sort'] == '999999') ? "" : "<input type=\"text\" name=\"options[formula][" . $id . "]\" style=\"width: 50px !important\" value=\"" . multiformHelper::secureString($o['formula']) . "\">") . "</td>
    ...
    public function GetOptionImageSizeRadioSettingsControl($name, $params)
    {
    ...
    {
        // Было ли что-либо отмечено; не передано ли пустое текстовое поле
        if (empty($post_fields[$form_field['id']]['formatted']) || (!empty($post_fields[$form_field['id']]['formatted']) && !empty($form_field['option_fields'][$post_fields[$form_field['id']]['formatted']]) && $form_field['option_fields'][$post_fields[$form_field['id']]['formatted']]['sort'] == 999999 && !$post_fields[$form_field['id']]['value'])) {
            return false;
        }
        return true;
    }
    ...
    {
        return array(
            'control_type' => 'GetRadioFieldControl',
            'title' => _w('New radio field'),
            'properties' => array(
                'title' => array(
                    'title' => _w('Title'),
                    'value' => '',
            ...
            'options' => array(
                'option_values' => array(
                    ...
                    'control_type' => waHtmlControl::CUSTOM . ' ' . 'GetOptionRadioSettingsControl',
                ),
                ...
                'allow_other' => array(
                    'label' => _w('Allow other'),
                    'class' => 'f-update-checkbox data-option-other',
                    'value' => 0,
                    'filter' => 'int',
                    'description' => _w('Add additional input text field as the last choice'),
                    'control_type' => waHtmlControl::CHECKBOX,
                ),
                'option_image_size' => array(
                    'title' => _w('Option image size'),
                    'title_type' => 'inline',
                    'control_type' => waHtmlControl::CUSTOM . ' ' . 'GetOptionImageSizeRadioSettingsControl',
                ),
            ),
            'sizes' => array(
                'block_width' => array(
