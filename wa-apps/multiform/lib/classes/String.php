...
class Hash
{
    ...
    public static function combine(array $data, $keyPath, $valuePath = null, $groupPath = null)
    {
        ...
        if (count($keys) !== count($vals)) {
            throw new \waException(_wp('Hash::combine() needs an equal number of keys + values'));
        }
    ...
    public static function get(array $data, $path, $default = null)
    {
        ...
        if (is_string($path) || is_numeric($path)) {
            ...
            if (!is_array($path)) {
                throw new \InvalidArgumentException(sprintf(_wp('Invalid Parameter %s, should be dot separated path or array'), var_export($path, true)));
            }
            $parts = $path;
        }
    ...
    public static function nest(array $data, $options = array())
    {
        ...
        if (!$return) {
            throw new \InvalidArgumentException(_wp('Invalid data array to nest'));
        }
    ...
    /**
     * Works like recursive array merge with operator +
     *
     * @return array
     */
    public static function array_merge_deep()
    {
        $args = func_get_args();
        return self::_array_merge_deep_array($args);
    }

    /**
     * Helper for array_merge_deep()
     *
     * @param array $arrays
     * @return array
     */
    private static function _array_merge_deep_array($arrays)
    {
        $result = array();
        foreach ($arrays as $array) {
            foreach ($array as $key => $value) {
                if (isset($result[$key]) && is_array($result[$key]) && is_array($value)) {
                    $result[$key] = self::_array_merge_deep_array(array(
                        $result[$key],
                        $value,
                    ));
                } else {
