...
;(function(factory) {
    ...
    $.Zebra_DatePicker = function(element, options) {
        ...
        plugin.setDefault = function(value) {
            var parts = value.split('-');
            if (parts.length === 3) {
                var year = parts[0],
                    month = parts[1],
                    day = parts[2],
                    default_date = new Date(year, month-1, day, 12, 0, 0),
                    selected_value = format(default_date);

                $element.val(selected_value);
                if (plugin.settings.onSelect && typeof plugin.settings.onSelect == 'function') {
                    plugin.settings.onSelect.call($element, selected_value, year + '-' + str_pad(month, 2) + '-' + str_pad(day, 2), default_date, $element, getWeekNumber(default_date));
                }
            }
        };
        ...
        plugin.show = function() {
            ...
            // get the default date, from the element, and check if it represents a valid date, according to the required format
            var default_date = check_date($element.val() || (plugin.settings.start_date ? plugin.settings.start_date : ''));
            if (!default_date && $element.attr('data-value') !== undefined) {
                var parts = $element.attr('data-value').split('-');
                if (parts.length === 3) {
                    default_date = new Date(parts[0], (parts[1] || 1) - 1, parts[2] || 1);
                }
            } 
